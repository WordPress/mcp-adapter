...
### Added
- `McpAdapter::instance()` now raises a `_doing_it_wrong()` notice when the code being executed is not the MCP Adapter plugin's own copy, pointing developers at the `Requires Plugins` header. It covers both the plugin being inactive and the plugin being active while a bundled copy elsewhere takes over, which Jetpack Autoloader allows whenever the bundled copy is newer. The notice is only visible with `WP_DEBUG` enabled and does not change runtime behavior.

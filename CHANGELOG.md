...
### Added
- `McpToolCallContext::arguments()` returns a defensive copy of original client arguments before callback normalization or filters, preserving JSON objects and arrays for input binding.
- Optional per-server `supported_protocol_versions` on `create_server()` and `McpServer` restricts request admission and advertised revisions. The default preserves both protocol families and legacy aliases.

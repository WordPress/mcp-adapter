 ...
 *
 * Note: McpErrorFactory returns revision-neutral error envelope arrays in wire shape.
 * These tests verify that the envelopes contain the correct data.
 */
final class ErrorEnvelopeTest extends TestCase {
	...
	public function test_error_envelopes_have_consistent_shape(): void {
		...
		// Verify it's an error envelope array
		$this->assertIsArray( $err );
		$this->assertArrayHasKey( 'error', $err );

		// Verify envelope values
		$this->assertSame( '2.0', $err['jsonrpc'] );
		$this->assertIsArray( $err['error'] );
		$this->assertNotNull( $err['error']['code'] );
		$this->assertNotNull( $err['error']['message'] );

		// Verify the envelope keeps a consistent structure
	...
	public function test_jsonrpc_message_validation_invalid_version(): void {
		...
		// Returns an error envelope array on failure

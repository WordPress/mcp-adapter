...
final class ToolsHandlerCallTest extends TestCase {
	...
	public function test_image_result_carries_meta(): void {
		...
		$this->assertSame( array( 'user' ), $block['annotations']['audience'] );
	}
	...
	/**
	 * @dataProvider data_image_results_without_usable_bytes
	 *
	 * @param array $shape Image-marked tool result.
	 */
	public function test_image_result_without_usable_bytes_falls_through_to_tool_data( array $shape ): void {
		$result = $this->call_tool_returning( $shape );
	 ...
	 */
	public function data_image_results_without_usable_bytes(): array {
		return array(
	...
	public function test_embedded_resource_rejects_invalid_annotations_dto(): void {
		$result = $this->call_tool_returning(
			array(
				'type'        => 'resource',
				'uri'         => 'WordPress://local/tool-embedded-text',
				'text'        => 'body',
				'annotations' => new Annotations( array( 'robot' ), 5.0, 'not-a-date' ),
			)
		);

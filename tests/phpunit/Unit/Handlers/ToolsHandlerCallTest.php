...
final class ToolsHandlerCallTest extends TestCase {
	...
	public function test_pre_tool_call_completion_preserves_generated_chain_invariants(): void {
		$execution_count = 0;
		$received_args   = array();
		$this->register_counted_tool( 'test/pre-tool-call-generated', $execution_count, $received_args );
		$handler = new ToolsHandler( $this->makeServer( array( 'test/pre-tool-call-generated' ) ) );

		for ( $seed = 0; $seed < 256; ++$seed ) {
			$chain_length  = $seed % 13;
			$terminal_index = ( $seed * 7 + 3 ) % ( $chain_length + 1 );
			$expected_args = array(
				'seed'    => $seed,
				'enabled' => 0 === $seed % 2,
				'empty'   => 0 === $seed % 5 ? null : '',
				'nested'  => array(
					'index'  => $seed % 17,
					'values' => array( $seed, -$seed, str_repeat( 'x', $seed % 31 ) ),
				),
			);
			$invoked         = array();
			$execution_before = $execution_count;
			$completed_result = CallToolResult::fromArray(
				array(
					'content' => array( array( 'type' => 'text', 'text' => 'Generated completion ' . $seed ) ),
					'isError' => false,
				)
			);

			$filter = static function ( array $callbacks ) use ( $chain_length, $terminal_index, $expected_args, $completed_result, &$invoked ): array {
				for ( $index = 0; $index < $chain_length; ++$index ) {
					$callbacks[] = static function ( array $args ) use ( $index, $terminal_index, $expected_args, $completed_result, &$invoked ): ?CallToolResult {
						if ( $args !== $expected_args ) {
							throw new \RuntimeException( 'Generated completion callback received mutated arguments.' );
						}

						$invoked[] = $index;

						return $index === $terminal_index ? $completed_result : null;
					};
				}
			...
			$result = $handler->call_tool(
				array(
					'params' => array(
						'name'      => 'test-pre-tool-call-generated',
						'arguments' => $expected_args,
					),
				)
			);

			if ( $terminal_index < $chain_length ) {
				$this->assertSame( $completed_result, $result, 'Terminal completion mismatch for seed ' . $seed );
				$this->assertSame( $execution_before, $execution_count, 'Executor ran after completion for seed ' . $seed );
				$this->assertSame( range( 0, $terminal_index ), $invoked, 'Callback order mismatch for seed ' . $seed );
			} else {
				$this->assertFalse( (bool) $result->getIsError(), 'Execution failed for seed ' . $seed );
				$this->assertSame( $execution_before + 1, $execution_count, 'Executor count mismatch for seed ' . $seed );
				$this->assertSame( $expected_args, $received_args, 'Executor arguments mismatch for seed ' . $seed );
				$this->assertSame( $chain_length > 0 ? range( 0, $chain_length - 1 ) : array(), $invoked, 'Callback order mismatch for seed ' . $seed );
			}

			remove_filter( 'mcp_adapter_pre_tool_call_completion_callbacks', $filter );
		}

		wp_unregister_ability( 'test/pre-tool-call-generated' );
	}

	public function test_pre_tool_call_completion_exception_prevents_execution(): void {
		$execution_count = 0;
		$received_args   = array();
		$this->register_counted_tool( 'test/pre-tool-call-exception', $execution_count, $received_args );
		$handler = new ToolsHandler( $this->makeServer( array( 'test/pre-tool-call-exception' ) ) );
		...
		$result = $handler->call_tool( array( 'params' => array( 'name' => 'test-pre-tool-call-exception' ) ) );
		...
		remove_filter( 'mcp_adapter_pre_tool_call_completion_callbacks', $filter );
		wp_unregister_ability( 'test/pre-tool-call-exception' );
	}

...
final class McpPromptValidatorTest extends TestCase {
	...
	// =========================================================================
	// validate_prompt_data Tests (minimal shapes)
	// =========================================================================

	public function test_validate_prompt_data_with_minimal_prompt(): void {
		$prompt = array(
			'name' => 'test-prompt',
		);
	...
	public function test_validate_prompt_instance_with_invalid_prompt(): void {
		...
		// because the Prompt DTO allows any name. Instead, test
		// validate_prompt_data directly with an invalid name.
		$result = McpPromptValidator::validate_prompt_data( array( 'name' => 'invalid/name' ) );
		$this->assertWPError( $result );
		$this->assertSame( 'mcp_prompt_validation_failed', $result->get_error_code() );
	}

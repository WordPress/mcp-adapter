...
final class McpToolTest extends TestCase {
	...
	public function test_fromArray_normalizes_a_stored_boolean_hint(): void {
		$tool = McpTool::fromArray(
			array(
				'name'        => 'loose-hint-tool',
				'handler'     => static fn( $args ) => array( 'ok' => true ),
				'annotations' => array( 'readOnlyHint' => '1' ),
			)
		);

		$this->assertNotWPError( $tool );
		$this->assertTrue( $tool->get_protocol_dto()->toArray()['annotations']['readOnlyHint'] );
	}
